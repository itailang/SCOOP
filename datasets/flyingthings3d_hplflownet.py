...
class FT3D(SceneFlowDataset):
    def __init__(self, root_dir, nb_points, all_points, mode, nb_examples):
        """
        ...
        mode : str
            ...
            'test': test dataset
        nb_examples: int
            Number of examples for the dataset. Active if > 0.

        """

        super(FT3D, self).__init__(nb_points, all_points)
    ...
    def load_sequence(self, idx):
        ...
        for fname in ["pc1.npy", "pc2.npy"]:
            pc = np.load(os.path.join(self.filename_curr, fname))
            pc[..., 0] *= -1
            pc[..., -1] *= -1
            sequence.append(pc)
        ground_truth = [
